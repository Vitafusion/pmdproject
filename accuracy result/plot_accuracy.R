...
p3 = dat %>% filter(m==3) %>% ggplot() + 
  ...
  scale_y_continuous(trans = 'log10', breaks = trans_breaks('log10', function(x) 10^x),labels = trans_format('log10', math_format(10^.x))) +labs(subtitle = 'm=3') + 
  ylab("MAE")
...
p4 = dat %>% filter(m==4) %>% ggplot() + 
  geom_path(aes(x=n,y=mae,colour=method,group=method,linetype=method), show.legend = F) + 
  scale_y_continuous(trans = 'log10', breaks = trans_breaks('log10', function(x) 10^x),labels = trans_format('log10', math_format(10^.x))) +labs(subtitle = 'm=4') + 
