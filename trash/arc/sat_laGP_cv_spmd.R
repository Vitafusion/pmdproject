...
## you will need to make a symbolic link to tpm-git
graceHe <- read.table("tpm-git/data/GRACE/graceHe.dat", header=TRUE)
...
#get mpi rank
rnk <- mpi.comm.rank(comm=0)
#get number of mpi ranks
np <- mpi.comm.size(comm=0)

## number of OpenMP threads
nth <- as.numeric(Sys.getenv("OMP_NUM_THREADS"))
print(paste("Threads on rank",rnk,"is:",nth),quote=FALSE)

#generate list of folds (only on master)
if (rnk==0) {
  all.folds <- cv.folds(nrow(X), np)
}

#get fold
o <- mpi.scatter.Robj(obj=all.folds,comm=0)
print(paste("Length of fold on rank",rnk,"is:",length(o)),quote=FALSE)

#run aGP on each rank
pout <- fold.do(o, X, Y, nth)
print(paste("Length of pout on rank",rnk,"is:",length(pout)),quote=FALSE)

#gather the results into a list on the master
all.outs <- mpi.gather.Robj(obj=pout,comm=0)

#do post-processing
if (rnk == 0) { 
  print(paste("Length of all.outs on rank",rnk,"is:",length(all.outs)),quote=FALSE)
  ...
  save(YY, Fi, scores, rmse, rmspe, file="sat_graceHe_laGP_cv_spmd.RData") #13
}
