...
# ## serial version ##
# 
# cv.serial(X=X, Y=Y, nth=nth, folds=5)
# 
# ## end serial version ##
...
np <- as.numeric(Sys.getenv("SLURM_NNODES"))

print(paste("Running parallel version with",np,"processes and",nth,"threads per process"),quote=FALSE)
...
#stop the cluster
## the next two lines should actually be the other way around, but
## there is a Rmpi/OpenMP > 2 incompatibility leading to hangs 
## (see https://stackoverflow.com/questions/41007564/stopcluster-in-r-snow-freeze)
## A solution is to flip them.  This causes a segfault, but at least the
## program terminates so you don't use all your allocation
mpi.exit()
stopCluster(cl)
